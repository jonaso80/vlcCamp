...
const FavoriteCamps: React.FC<FavoriteCampsProps> = ({ user }) => {
  ...
  useEffect(() => {
    const loadFavorites = async () => {
      try {
        // Limpiar favoritos antiguos que apuntan a campamentos que ya no existen
        // (camp_id 1-9 son los campamentos de ejemplo que se eliminaron)
        const { data: oldFavorites, error: fetchError } = await supabase
          .from('favorites')
          .select('id, camp_id')
          .eq('user_id', user.email)
          .in('camp_id', [1, 2, 3, 4, 5, 6, 7, 8, 9]);

        if (fetchError) {
          console.error('Error al cargar favoritos desde Supabase', fetchError);
        } else if (oldFavorites && oldFavorites.length > 0) {
          // Eliminar favoritos antiguos
          const idsToDelete = oldFavorites.map(f => f.id);
          ...
          if (deleteError) {
            console.error('Error al eliminar favoritos antiguos', deleteError);
          } else {
            console.log('Favoritos antiguos eliminados:', idsToDelete.length);
          }
        }

        // Los campamentos se cargarán desde Supabase cuando estén contratados
        // Por ahora, no hay campamentos disponibles
        setCamps([]);
      } catch (error) {
