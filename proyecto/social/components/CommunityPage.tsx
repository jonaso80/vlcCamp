...
const CommunityPage: React.FC<CommunityPageProps> = ({ currentUser, onSwitchAccount, onAccountClick }) => {
    const { t } = useTranslations();
    const storagePostsKey = 'vlcCampCommunityPosts';
    const storageStoriesKey = 'vlcCampCommunityStories';
    const storageRoleKey = useMemo(() => currentUser ? `vlcCampCommunityRole_${currentUser.email}` : null, [currentUser]);
    const monitorCampId = useMemo(() => currentUser ? localStorage.getItem(`vlcCampMonitorCamp_${currentUser.email}`) : null, [currentUser]);

    const [role, setRole] = useState<CommunityUserRole | null>(() => storageRoleKey ? localStorage.getItem(storageRoleKey) as CommunityUserRole | null : null);
    
    const [posts, setPosts] = useState<Post[]>(() => {
      try {
        const s = localStorage.getItem(storagePostsKey);
    ...
    const [stories, setStories] = useState<Story[]>(() => {
      try {
        const s = localStorage.getItem(storageStoriesKey);
        return s ? JSON.parse(s) : [];
      } catch {
        return [];
      }
    ...
    useEffect(() => { localStorage.setItem(storagePostsKey, JSON.stringify(posts)); }, [posts]);
    useEffect(() => { localStorage.setItem(storageStoriesKey, JSON.stringify(stories)); }, [stories]);
    ...
    const handleAddPost = (postData: { type: 'photo' | 'poll' | 'text', caption: string, imageBase64?: string | null, poll?: { question: string, options: string[] } }) => {
        try {
            if (!currentUser || !monitorCampId) return;
            const basePost = {
                id: Date.now(),
                campId: parseInt(monitorCampId, 10),
                monitorId: 0,
                monitorName: currentUser.name,
                monitorAvatar: currentUser.avatar,
            ...
            if (postData.type === 'photo' && postData.imageBase64) {
                newPost = { ...basePost, type: 'photo', imageUrl: postData.imageBase64 };
